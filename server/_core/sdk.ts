...
class SDKServer {
  private getSessionSecret() {
    return new TextEncoder().encode(ENV.cookieSecret || "fallback-dev-secret-32chars!!!!!");
  }
  ...
  async createSessionToken(openId: string, options: { expiresInMs?: number; name?: string } = {}): Promise<string> {
    ...
    return new SignJWT({ openId, appId: "tiopatinhas", name: options.name || "" })
  ...
  async authenticateRequest(req: Request): Promise<User> {
    const session = await this.verifySession(this.parseCookies(req.headers.cookie).get(COOKIE_NAME));
