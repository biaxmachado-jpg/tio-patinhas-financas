    ...
    if (cardTransactions.length > 0) {
      ...
      for (const ct of cardTransactions) {
        try {
          ...
          // New: id, userId, cardId, categoryId, description, amount, date, dueDate, installments, currentInstallment, paid, paidAt, notes, createdAt, updatedAt
          ...
          await targetConn.execute(
            "INSERT INTO creditCardTransactions (id, userId, cardId, categoryId, description, amount, date, dueDate, installments, currentInstallment, paid, paidAt, notes, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE description=?, amount=?, date=?, dueDate=?, installments=?, currentInstallment=?, paid=?, paidAt=?, notes=?, updatedAt=?",
            values
          );
          migrated++;
        } catch (error) {
