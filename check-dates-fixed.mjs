...
async function checkDates() {
  ...
  try {
    ...
    const [dueDateDistribution] = await targetConn.execute(
      "SELECT DATE_FORMAT(dueDate, '%Y-%m') as mes, COUNT(*) as total FROM creditCardTransactions WHERE cardId = 1 GROUP BY DATE_FORMAT(dueDate, '%Y-%m') ORDER BY mes DESC LIMIT 10"
    );
    ...
    const [dateDistribution] = await targetConn.execute(
      "SELECT DATE_FORMAT(date, '%Y-%m') as mes, COUNT(*) as total FROM creditCardTransactions WHERE cardId = 1 GROUP BY DATE_FORMAT(date, '%Y-%m') ORDER BY mes DESC LIMIT 10"
    );
