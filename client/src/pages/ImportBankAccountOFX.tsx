...
export default function ImportBankAccountOFX() {
  ...
  const accountQuery = trpc.bankAccounts.get.useQuery({ id: parseInt(accountId || "0") });
  const importMutation = trpc.transactions.importFromOFX.useMutation();

  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const selectedFile = e.target.files?.[0];
    if (selectedFile) {
      if (!selectedFile.name.endsWith(".ofx") && !selectedFile.name.endsWith(".txt")) {
        toast.error("Por favor, selecione um arquivo OFX ou TXT");
        return;
      }
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground mb-4">
                  Faça upload do arquivo OFX da sua conta bancária. O sistema irá extrair as transações automaticamente.
                </p>
              </div>
              ...
              <div className="border-2 border-dashed border-border rounded-lg p-8 text-center hover:border-primary transition-colors cursor-pointer">
                ...
                <label htmlFor="ofx-upload" className="cursor-pointer">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Apenas arquivos OFX ou TXT são aceitos (máx. 10MB)
                  </p>
                </label>
              </div>
            ...
            <div className="bg-amber-50 dark:bg-amber-950 border border-amber-200 dark:border-amber-800 rounded-lg p-4 flex gap-3">
              ...
              <div className="text-sm text-amber-800 dark:text-amber-200">
                ...
                <p>
                  O sistema irá extrair automaticamente as transações do arquivo OFX. Certifique-se de que o arquivo está no formato correto.
                </p>
              </div>
            </div>
            ...
            <div className="flex gap-3 justify-end">
              ...
              >
                {isLoading ? "Processando..." : "Importar OFX"}
              </Button>
            </div>
          </form>
        )}
